...
$total_query = "SELECT COUNT(*) as total FROM nominations_tb";
$total_result = mysqli_query($conn, $total_query);
$total_row = mysqli_fetch_assoc($total_result);
$total_nominations = $total_row['total'];
$total_pages = ceil($total_nominations / $limit);

// Fetch nominations for current page
$stmt = $conn->prepare("SELECT * FROM nominations_tb ORDER BY id DESC LIMIT ? OFFSET ?");
$stmt->bind_param("ii", $limit, $offset);
$stmt->execute();
$result = $stmt->get_result();
$nominations = $result->fetch_all(MYSQLI_ASSOC);
$stmt->close();
$conn->close();
?>
...
<head>
    ...
    <title>All Nominations - Mzehemen U Tiv</title>
    <style>
      body {
...
<body>
  ...
  <div class="container">
      ...
      <h2 class="mb-4">All Nominations</h2>
